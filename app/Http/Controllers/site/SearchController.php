    ...
    public function search(Request $request)
    {
        ...
        if (!empty($term)) {
            if (curlang() == '_en') {
                ...
                $announcementResults = Announcement::where('name_en', 'like', '%' . $term . '%')->orWhere('name_sw', 'like', '%' . $term . '%')->orWhere('content_en', 'like', '%' . $term . '%')->limit(50)->orderBy('id','DESC')->get();
                ...
                $passengerResults = Passenger::where('title_en', 'like', '%' . $term . '%')->orWhere('title_sw', 'like', '%' . $term . '%')->orWhere('content_en', 'like', '%' . $term . '%')->limit(50)->orderBy('id','DESC')->get();
                ...
                $announcementResults = Announcement::where('name_en', 'like', '%' . $term . '%')->orWhere('name_sw', 'like', '%' . $term . '%')->orWhere('content_sw', 'like', '%' . $term . '%')->limit(50)->orderBy('id','DESC')->get();
                ...
                $passengerResults = Passenger::where('title_en', 'like', '%' . $term . '%')->orWhere('title_sw', 'like', '%' . $term . '%')->orWhere('content_sw', 'like', '%' . $term . '%')->limit(50)->orderBy('id','DESC')->get();

            }
        }

        //return results to search page...
        return view('site.search.index', compact('term', 'pressResults', 'speechResults', 'publicationResults', 'newsResults','announcementResults','faqResults', 'pageResults','eventResults','administrationResults','howdoiResults','passengerResults'));
