        ...
        foreach ($parent_items as $parent) {
            ...
            if($child_items->count() > 0){
                ...
                elseif ($menu_name == 'mega_menu') {
                    ...
                    $menu_string .= "<li class='nav-item dropdown'><a href='#' class='nav-link dropdown-toggle' data-toggle='dropdown'>".htmlentities($parent->title)."</a><ul class='dropdown-menu'>";
                }
                    ...
                    if($grands->count()){
                        $menu_string .= "
                                <li class='nav-item dropdown-submenu'>
                                    <a class='dropdown-item dropdown-toggle' data-toggle='dropdown' href='#'>".htmlentities($child->title)."</a>"."
                                    <ul class='dropdown-menu'>";

                        foreach ($grands as $grand) {
                            if ($grand->url_external !== null) {
                                $menu_string .= "<li><a target='_blank' rel='noopener noreferrer' class='dropdown-item' href= '".url($grand->url)."'>".htmlentities($grand->title)."</a></li>";
                            }else{
                                $menu_string .= "<li><a class='dropdown-item' href= '".url($grand->url)."'>".htmlentities($grand->title)."</a></li>";
                            }
                        }
                        ...
                        if ($child->url_external !== null) {
                            $menu_string .= "<li><a target='_blank' rel='noopener noreferrer' class='dropdown-item' href= '".url($child->url)."'>".htmlentities($child->title)."</a></li>";
                        }else{
                            $menu_string .= "<li><a class='dropdown-item' href= '".url($child->url)."'>".htmlentities($child->title)."</a></li>";
                        }
                    }
                }
                $menu_string .= "</ul></li>";
                ...
                elseif ($menu_name == 'mega_menu') {
                    ...
                    // open new tab for external url
                    if ($parent->url_external !== null) {
                        $menu_string.= "<li class='nav-item'><a class='nav-link' target='_blank' rel='noopener noreferrer' href='".url($parent->url)."'>".htmlentities($parent->title)."</a></li>";
                    }else {
                        $menu_string.= "<li class='nav-item'><a class='nav-link' href='".url($parent->url)."'>".htmlentities($parent->title)."</a></li>";
                    }
                }
